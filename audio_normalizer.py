...
def process_single_file(args):
    ...
    try:
        ...
        return f"❌ {filename}: Errore -> {str(e)}"

def main():
    # Setup degli argomenti da terminale
    parser = argparse.ArgumentParser(description="Normalizza l'audio dei video in una cartella.")
    parser.add_argument("folder", nargs="?", help="Percorso della cartella contenente i video")
    args = parser.parse_args()

    # Carica config per i parametri di normalizzazione (LUFS, Peak, ecc.)
    cfg = load_config()
    ...
    if args.folder:
        ...
        # Fallback al config se non viene passato un parametro
