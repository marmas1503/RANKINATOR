...
def timestamp_to_seconds(ts):
    """Converte formato MM:SS in secondi (float)."""
    ...
    try:
        ...
        return float(ts) # Se è già un numero
    except:
        return None
...
def process_video(url, rank, manual_ts=None):
    ...
    try:
        ...
        if (start_highlight + dur_b) > duration_total:
            ...
            print(f"⚠️ Rank {rank}: Fine video vicina. Start spostato a {nuovo_start:.1f}s")
            start_highlight = nuovo_start

        # 5. Esecuzione Tagli
        if (start_intro + dur_a >= start_highlight):
            print(f"⚠ Rank {rank}: Clip unica...")
            ydl_opts_direct = {
                'format': 'bestvideo[ext=mp4][height<=1080]+bestaudio[ext=m4a]/best[ext=mp4]',
                'outtmpl': output_path, 'quiet': True,
                'external_downloader': 'ffmpeg',
            ...
            print(f"🎬 Rank {rank}: Generazione Crossfade...")
...
if __name__ == "__main__":
    ...
    # Prendi indice colonna timestamp (se definita)
